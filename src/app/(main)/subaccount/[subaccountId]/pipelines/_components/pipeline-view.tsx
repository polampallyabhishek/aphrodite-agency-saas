...
const PipelineView = ({
  ...
  const ticketsFromAllLanes: TicketAndTags[] = React.useMemo(
    () => lanes.flatMap((item) => item.Tickets),
    [lanes]
  );
  const [allTickets, setAllTickets] = React.useState(ticketsFromAllLanes);

  const handleAddLane = React.useCallback(() => {
    setOpen(
      <CustomModal
        title=" Create A Lane"
